...
export function BlogCard({ title, category, excerpt, href, image }: BlogCardProps) {
  return (
    <Link href={href} className="group block overflow-hidden rounded-2xl border border-white/10 bg-charcoal transition duration-500 hover:-translate-y-1 hover:border-gold/40">
      ...
      <div className="p-6">
        <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-gold">{category}</p>
        <div className="mt-2 flex items-start justify-between gap-4">
          <div>
            <h3 className="text-xl font-bold tracking-tight text-white sm:text-2xl">{title}</h3>
            <p className="mt-2 text-sm font-normal leading-6 text-white/55">{excerpt}</p>
          </div>
