...
// ============================================================
export function getPayuKey() {
    return process.env.PAYU_MERCHANT_KEY || "";
}
export function getPayuSalt() {
...
export function generatePayuRequestHash(params) {
    ...
    if (!params.txnid || !params.txnid.trim())
        throw new Error("PayU txnid is missing");
    if (params.amount === undefined || params.amount === null || params.amount === "") {
        throw new Error("PayU amount is missing");
    }
    if (!params.productinfo || !params.productinfo.trim())
        throw new Error("PayU productinfo is missing");
    if (!params.firstname || !params.firstname.trim())
        throw new Error("PayU firstname is missing");
    if (!params.email || !params.email.trim())
        throw new Error("PayU email is missing");
    ...
    if (process.env.NODE_ENV !== "production") {
        ...
        console.log(`[PayU Diagnostic] txnid: ${params.txnid.trim()}, amount: ${formattedAmount}, productinfo: ${params.productinfo.trim()}, firstname: ${params.firstname.trim()}, email: ${params.email.trim()}, udf1: ${udf1}, udf2..udf5: empty, delimiters: ${delimiterCount}, hashLength: ${hash.length}`);
    }
    return hash;
}
// ============================================================
