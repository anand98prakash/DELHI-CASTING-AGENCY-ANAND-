...
async function activateUserEntitlement(
  ...
  if (!isAlreadyLinked) {
    await createNotification({
      ...
      message: `Your 3-month ${
        plan === "BRAND_PREMIUM" ? "Brand" : "Artist"
      } Premium entitlement is active until ${expiresAt.toLocaleDateString(
        "en-IN",
  ...
  try {
    ...
    if (!targetOrderId || !targetPaymentId || !targetSignature) {
      res.status(400).json({
        success: false,
        message:
          "Missing required payment verification credentials (order ID, payment ID, signature)",
      });
      return;
    }
    ...
    if (
      targetPaymentId.startsWith("pay_dca_") ||
    ...
    if (payment.status === "PAID") {
      const { entitlement } = await activateUserEntitlement(
        payment.userId,
        payment.id,
        payment.plan,
        payment.amount,
