...
export async function createArtistProfile(req, res) {
    try {
        ...
        const { fullName, phone, gender, dateOfBirth, city, state, bio, height, weight, chest, waist, hips, languages, skills, specialAbilities, profilePhoto, headshots, } = req.body;
        // Required field
        if (!fullName?.trim()) {
            res.status(400).json({
                success: false,
...
// ==========================================
export async function uploadArtistPhotos(req, res) {
    try {
        if (!req.user) {
            res.status(401).json({
                success: false,
